    ...
    fun registerIfAbsent(oAuth2UserInfo: OAuth2UserInfo): SocialMember {
        return if (!socialUserRepository.existsByProviderAndProviderId(oAuth2UserInfo.provider, oAuth2UserInfo.id)) {
            val socialUser = when(oAuth2UserInfo.provider){
                OAuth2Provider.KAKAO -> SocialMember.ofKakao(oAuth2UserInfo.id.toLong(), oAuth2UserInfo.email, oAuth2UserInfo.nickname)
                OAuth2Provider.GOOGLE -> SocialMember.ofGoogle(oAuth2UserInfo.id.toLong(), oAuth2UserInfo.email, oAuth2UserInfo.nickname)
            }
